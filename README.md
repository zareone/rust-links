# Rust Links

## Books and Guides

[The Rust Programming Language](http://doc.rust-lang.org/book/)

[The Rust Standard Library](http://doc.rust-lang.org/std/)

[Rust by Example](http://rustbyexample.com/)



## Ownership and Lifetimes

[Rust Means Never Having to Close a Socket](http://blog.skylight.io/rust-means-never-having-to-close-a-socket/)

[Rust ownership, the hard way](http://chrismorgan.info/blog/rust-ownership-the-hard-way.html)

[Rust lifetimes: Getting away with things that would be reckless in C++](http://www.randomhacks.net/2014/09/19/rust-lifetimes-reckless-cxx/)

[Explore the ownership system in Rust](http://nercury.github.io/rust/guide/2015/01/19/ownership.html)

[Understanding Pointers, Ownership, and Lifetimes in Rust](http://paulkoerbitz.de/posts/Understanding-Pointers-Ownership-and-Lifetimes-in-Rust.html)

[Rust's Ownership model for JavaScript developers](http://blog.thoughtram.io/rust/2015/05/11/rusts-ownership-model-for-javascript-developers.html)



# Type system

[The Option Type](http://blog.8thlight.com/dave-torre/2015/03/11/the-option-type.html)

[Using The Option Type Effectively](http://blog.8thlight.com/uku-taht/2015/04/29/using-the-option-type-effectively.html)

[Virtual Structs Part 1: Where Rust’s Enum Shines](http://smallcultfollowing.com/babysteps/blog/2015/05/05/where-rusts-enum-shines)

[Why your first FizzBuzz implementation may not work](http://chrismorgan.info/blog/rust-fizzbuzz.html)

[Rust's mem::transmute and void pointers](http://nercury.github.io/rust/pointers/2014/10/30/rust-mem-transmute-and-void-pointers.html)

[Peeking inside Trait Objects](http://huonw.github.io/blog/2015/01/peeking-inside-trait-objects/)

[The Sized Trait](http://huonw.github.io/blog/2015/01/the-sized-trait/)

[Abstraction without overhead: traits in Rust](http://blog.rust-lang.org/2015/05/11/traits.html)


# Error handling

[Error handling in Rust: a k-NN case study](http://huonw.github.io/blog/2014/06/error-handling-in-rust-knn-case-study/)

[Error Handling in Rust](http://blog.burntsushi.net/rust-error-handling/)

[Improved Error Handling in Rust](http://lucumr.pocoo.org/2014/11/6/error-handling-in-rust/)


# Web Development

[Deploying Rust applications to Heroku, with example code for Iron](http://www.randomhacks.net/2014/09/17/deploying-rust-heroku-iron/)

[24 Days of Rust - Hyper](http://siciarz.net/24-days-of-rust-hyper/)


# Architecture

[Dependency injection container - Figuring out the design - A naive start](http://nercury.github.io/rust/di/2014/11/02/building-dependency-injection-container-in-rust.html)

[Dependency injection container - Learning the ropes in Rust](http://nercury.github.io/rust/di/2015/01/02/dependency-injection-learning-rust.html)


# Concurrency patterns


# Interfacing

[Raw system calls for Rust](http://mainisusuallyafunction.blogspot.com/2014/09/raw-system-calls-for-rust.html)

[Calling a Rust library from C (or anything else!)](http://mainisusuallyafunction.blogspot.com/2014/08/calling-rust-library-from-c-or-anything.html)

[A taste of Rust (yum) for C/C++ programmers](http://mainisusuallyafunction.blogspot.com/2014/10/a-taste-of-rust-yum-for-cc-programmers_29.html)


# Algorithms

[Comparing k-NN in Rust](http://huonw.github.io/blog/2014/06/comparing-knn-in-rust/)


# Other

[Quick tip: the #[cfg_attr] attribute](http://chrismorgan.info/blog/rust-cfg_attr.html)

[Turing tarpits in Rust's macro system](http://mainisusuallyafunction.blogspot.com/2015/02/turing-tarpits-in-rusts-macro-system.html)

[151-byte static Linux binary in Rust](http://mainisusuallyafunction.blogspot.com/2015/01/151-byte-static-linux-binary-in-rust.html)

[Type-Directed TDD in Rust](https://github.com/FranklinChen/type-directed-tdd-rust)


[Some notes on Send and Sync](http://huonw.github.io/blog/2015/02/some-notes-on-send-and-sync/)

[Rust infrastructure can be your infrastructure](http://huonw.github.io/blog/2015/03/rust-infrastructure-can-be-your-infrastructure/)

[Object Safety](http://huonw.github.io/blog/2015/01/object-safety/)

[Where Self Meets Sized: Revisiting Object Safety](http://huonw.github.io/blog/2015/05/where-self-meets-sized-revisiting-object-safety/)

[Finding Closure in Rust](http://huonw.github.io/blog/2015/05/finding-closure-in-rust/)
